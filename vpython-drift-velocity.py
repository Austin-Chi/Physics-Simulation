...
while True:
    ...
    fi=2*pi*np.random.rand(N)
    x,y,z=vrms*np.sin(theta)*np.cos(fi),vrms*np.cos(theta)*np.cos(fi),vrms*np.sin(theta)*np.sin(fi)
    v_array[collide]=np.transpose([x[collide],y[collide],z[collide]])
    for i in collide:
        if i ==True:
            total_c+=1
    ...
    if int(t/dt)%2000 == 0:
        tau = t*N/total_c # need to be modified
