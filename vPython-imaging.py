...
for angle in range(-7, 2):
	...
	ray.v = vector (cos(pi-angle/40.0), sin(pi-angle/40.0), 0)
	dt = 0.002
	while True:
		rate(1000)
		ray.pos = ray.pos + ray.v*dt
		# your code here
		if mag(ray.pos - g1center)<4 and ray.pos.x>0 and inside==False:
			inside = True
			ray.v=refraction_vector(1, nglass, ray.v, -norm(ray.pos - g1center))
		if mag(ray.pos - g2center)>4 and ray.pos.x<0 and inside==True:
			inside=False
			ray.v = refraction_vector(nglass, 1, ray.v, norm(ray.pos - g2center))
		if ray.pos.x <=-6:
			print(ray.pos.y)
			break
