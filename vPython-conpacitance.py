...
def solve_laplacian(u, u_cond, h, Niter=5000):
	...
	for i in range(Niter):
		V[u_cond] = u[u_cond]
		V[1:-1, 1:-1] = 1/4*(V[2:, 1:-1]+V[:-2, 1:-1]+V[1:-1, 2:]+V[1:-1, :-2]) # replace this 0 by your Laplacian Solver
	return V
...
psi = 0
D = 3
for i in range(int(N/2)-int(L/h/2.0)-D, int(N/2)+int(L/h/2.0)+D):
    psi+=(Ey[i, int(N/2)+int(d/h/2.0)+D]-Ey[i, int(N/2)+int(d/h/2.0)-D])*h
for i in range(int(N/2)+int(d/h/2.0)-D, int(N/2)+int(d/h/2.0)+D):
    psi+=(Ex[int(N/2)+int(L/h/2.0)+D, i]-Ex[int(N/2)-int(L/h/2.0)-D, i])*h
Q=psi * epsilon
C_ni=Q/V0
...
print("Q: %.3e"%Q)
print("ideal: %.3e"%C_i)
print("non ideal: %.3e"%C_ni)
print("error percentage: %.3f"%((C_ni-C_i)/C_i*100))